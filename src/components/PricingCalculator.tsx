...
export default function PricingCalculator() {
  ...
  return (
    <section id="pricing" className="relative py-24 sm:py-32 px-4 sm:px-6 lg:px-8 bg-omni-forest-deep">
      {/* Top divider */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-omni-mint/30 to-transparent" />
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          >
            {/* Module Count Slider */}
            <div className="bg-omni-forest/60 border border-white/5 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <Label className="text-sm text-white tracking-wide">{t('pricing.moduleCount')}</Label>
                <span className="text-lg font-light text-omni-mint tabular-nums">{moduleCount}</span>
              </div>
              <Slider
                value={[moduleCount]}
                min={1}
                ...
                className="w-full [&_[data-slot=slider-track]]:bg-white/10 [&_[data-slot=slider-range]]:bg-gradient-to-r [&_[data-slot=slider-range]]:from-omni-mint [&_[data-slot=slider-range]]:to-omni-mint-soft [&_[data-slot=slider-thumb]]:bg-omni-mint [&_[data-slot=slider-thumb]]:border-omni-mint [&_[data-slot=slider-thumb]]:h-5 [&_[data-slot=slider-thumb]]:w-5"
              />
              <div className="flex justify-between mt-2 text-xs text-omni-cream">
                <span>1</span>
                <span>6</span>
              </div>
              <p className="text-xs text-omni-cream mt-3">
            ...
            <div className="bg-omni-forest/60 border border-white/5 rounded-xl p-6">
              ...
              <Select value={moduleType} onValueChange={(v) => setModuleType(v as ModuleType)}>
                <SelectTrigger aria-label={t('pricing.moduleType')} className="w-full bg-omni-forest-deep border border-white/10 text-white h-11 hover:border-omni-mint/30 focus:border-omni-mint [&_svg]:text-omni-mint">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="bg-omni-forest/60 border border-white/5 rounded-xl p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-omni-cream mt-1">
                ...
                <Switch
                  ...
                  className="data-[state=checked]:bg-omni-mint data-[state=unchecked]:bg-white/10 w-11 h-6 [&>span]:h-5 [&>span]:w-5"
                />
              </div>
            </div>
                  ...
                  <div key={extra.id} className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <Checkbox
                        ...
                        className="data-[state=checked]:bg-omni-mint data-[state=checked]:border-omni-mint data-[state=checked]:text-omni-forest-deep border-white/20 h-4 w-4"
                      />
                      <Label
                        htmlFor={extra.id}
                        className="text-sm text-omni-cream cursor-pointer hover:text-white transition-colors"
                      >
                        {extra.label}
                      </Label>
                    </div>
                    <span className="text-xs text-omni-mint/70 tabular-nums shrink-0">
            ...
            <div className="bg-omni-forest/60 border border-white/5 rounded-xl p-6">
              ...
              <Select value={location} onValueChange={(v) => setLocation(v as Location)}>
                <SelectTrigger aria-label={t('pricing.location')} className="w-full bg-omni-forest-deep border border-white/10 text-white h-11 hover:border-omni-mint/30 focus:border-omni-mint [&_svg]:text-omni-mint">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-omni-forest border border-white/10 text-white">
          ...
          >
            {/* Total Price Card */}
            <div className="bg-omni-forest/80 border border-omni-mint/20 rounded-xl p-8 text-center mb-6 relative overflow-hidden">
              {/* Decorative background */}
              <div className="absolute inset-0 bg-gradient-to-br from-omni-mint/5 via-transparent to-transparent pointer-events-none" />
              <div className="relative">
                <div className="flex items-center justify-center gap-2 mb-4">
                  <Calculator size={20} className="text-omni-mint" />
                  <span className="text-xs tracking-[0.2em] text-omni-mint uppercase">
                ...
                <p className="text-xs text-omni-cream mt-3 tracking-wide">
                  {t('pricing.unbinding')}
                </p>
              </div>
            </div>
            ...
            <div className="bg-omni-forest/60 border border-white/5 rounded-xl p-6 flex-1">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between py-2 border-b border-white/5">
                  <span className="text-sm text-omni-cream">
                ...
                {calculation.upperFloorAddition > 0 && (
                  ...
                  >
                    <span className="text-sm text-omni-cream">{t('pricing.upperFloor')}</span>
                    <span className="text-sm text-white tabular-nums">
                ...
                {selectedExtras.size > 0 && (
                  <div className="py-2 border-b border-white/5">
                    <span className="text-sm text-omni-cream">{t('pricing.equipment')}</span>
                    <div className="mt-2 space-y-1 pl-3">
                      {EXTRAS.filter((e) => selectedExtras.has(e.id)).map((extra) => (
                        <div key={extra.id} className="flex items-center justify-between">
                          <span className="text-xs text-omni-cream/70">{extra.label}</span>
                          <span className="text-xs text-omni-cream tabular-nums">
                            +{formatEuro(extra.price)}
                          </span>
                        </div>
                      ))}
                ...
                {calculation.locationAddition > 0 && (
                  <motion.div
                    ...
                    className="flex items-center justify-between py-2 border-b border-white/5"
                  >
                    <span className="text-sm text-omni-cream">{t('pricing.citySurcharge')}</span>
                    <span className="text-sm text-white tabular-nums">
                ...
                <div className="flex items-center justify-between pt-4">
                  ...
                  <span className="text-lg font-light text-omni-mint tabular-nums">
                    <AnimatedNumber value={calculation.total} />
                  </span>
                </div>
              </div>
            ...
            <button
              ...
              className="mt-6 w-full py-4 rounded-xl bg-gradient-to-r from-omni-mint to-omni-mint-deep hover:from-omni-mint-soft hover:to-omni-mint text-omni-forest-deep font-medium tracking-[0.1em] uppercase text-sm transition-all duration-500 flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-[#C3F8BD]/20"
            >
