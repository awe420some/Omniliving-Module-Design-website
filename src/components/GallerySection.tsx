...
export default function GallerySection() {
  ...
  return (
    <section id="gallery" className="relative py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-omni-forest-deep">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            <p className="text-xs tracking-[0.3em] text-omni-mint uppercase">
              {t('gallery.label')}
            </p>
            <span className="text-[10px] text-omni-cream/50 tracking-wider">
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-3 sm:gap-4 space-y-3 sm:space-y-4">
          {galleryImages.map((image, index) => (
            ...
            >
              <div className={`relative rounded-lg overflow-hidden border border-white/5 transition-all duration-500 group-hover:scale-[1.02] group-hover:border-omni-mint/20 group-hover:shadow-lg group-hover:shadow-black/30 ${aspectClasses[image.aspect]}`}>
                <Image
                  src={image.src}
                  alt={image.title}
                  fill
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-end">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full border-2 border-omni-mint/50 flex items-center justify-center bg-black/30 backdrop-blur-sm transition-all duration-500 scale-50 group-hover:scale-100 opacity-0 group-hover:opacity-100" aria-label={image.title}>
                    <Search size={20} className="text-omni-mint" aria-hidden="true" />
                  </div>
                  ...
                  <div className="p-4 sm:p-6 w-full">
                    ...
                    <p className="text-xs text-omni-mint mt-1 tracking-wide">
      ...
      <Dialog open={isLightboxOpen} onOpenChange={setIsLightboxOpen}>
        <DialogContent
          className="sm:max-w-5xl bg-omni-forest-deep border-white/10 p-0 overflow-hidden"
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:border-omni-mint/30 transition-all duration-300"
          ...
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:border-omni-mint/30 transition-all duration-300"
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:border-omni-mint/30 transition-all duration-300"
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="w-full px-6 py-5 border-t border-white/5">
              ...
              <p className="text-sm text-omni-cream mt-1 leading-relaxed">
                {currentImage.description}
              </p>
              <p className="text-xs text-omni-mint/60 mt-2 tracking-wide">
