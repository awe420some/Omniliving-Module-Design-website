...
function PartnerLogo({ partner, index, t }: { partner: typeof partnerKeys[number]; index: number; t: (key: string) => string }) {
  return (
    ...
    >
      <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-xl bg-[#12121f]/60 border border-white/5 flex flex-col items-center justify-center gap-1 px-3 transition-all duration-500 group-hover:border-[#c9a96e]/30 group-hover:bg-[#12121f]/80 group-hover:shadow-lg group-hover:shadow-[#c9a96e]/5 cursor-pointer">
        {/* Initials badge */}
        <span className="text-xs font-bold tracking-[0.25em] text-[#8888a8] group-hover:text-[#c9a96e] transition-colors duration-500 uppercase">
          {partner.initials}
        </span>
        {/* Full name */}
        ...
        {/* Gold accent line */}
        <div className="w-6 h-[1px] bg-[#8888a8]/30 group-hover:bg-[#c9a96e]/60 transition-colors duration-500 mt-1" />
      </div>
